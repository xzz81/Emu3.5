#!/usr/bin/env python3
"""Build 2D target+distractor reference images for forced-position probing."""
...
OUT_DIR = Path("assets/ume_image_read_distractor_seed29")
SIZE = 512
SHAPE_SIZE = 104
BG = (148, 148, 148)
OUTLINE = (30, 30, 30)
...
def main() -> None:
    ...
    rows = []
    for obj_idx, (object_phrase, kind, color) in enumerate(OBJECTS):
        distractor_phrase, distractor_kind, distractor_color = OBJECTS[(obj_idx + 1) % len(OBJECTS)]
        ...
        for position_phrase in POSITIONS:
            ...
            draw_shape(draw, distractor_kind, bbox(distractor_position), distractor_color)
    ...
    print(f"[INFO] wrote {len(rows)} target+distractor images to {OUT_DIR}")
