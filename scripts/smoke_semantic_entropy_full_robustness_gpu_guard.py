#!/usr/bin/env python3
"""Smoke-test that full robustness stops at GPU availability preflight."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default="outputs/semantic_entropy_umm/full_robustness_gpu_guard_smoke")
...
if [[ "$*" == *"--query-gpu=index,name,memory.total,memory.used,utilization.gpu"* ]]; then
  for i in 0 1 2 3 4 5 6 7; do
    printf "%s, NVIDIA A800 80GB PCIe, 81920, 81919, 0\\n" "$i"
  done
  exit 0
fi
...
printf "fake nvidia-smi for semantic entropy GPU guard smoke\\n"
""",
        encoding="utf-8",
    )
    path.chmod(0o755)
...
def write_report(out_dir: Path, payload: dict[str, Any]) -> None:
    lines = [
        "# Full Robustness GPU Guard Smoke",
        "",
        f"Status: {payload['status']}",
        "",
        "This smoke test simulates a completed manual annotation table and a busy 8-GPU host. It verifies that a requested full robustness run stops at preflight instead of launching model workers.",
        "",
        "## Summary",
        "",
        ...
        f"- GPU availability OK: `{payload.get('gpu_availability_ok_for_launch')}`",
        f"- Busy GPUs: `{payload.get('busy_gpu_indices')}`",
    ...
    (out_dir / "full_robustness_gpu_guard_smoke_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="semantic_entropy_full_robustness_guard_"))
    ...
    try:
        ...
        nested_output = command.get("stdout_tail", "") + command.get("stderr_tail", "")
        for item in gate_report.get("commands", []):
            nested_output += item.get("stdout_tail", "") + item.get("stderr_tail", "")
        gpu_error_seen = "GPU availability is not OK" in nested_output
        status = (
            "PASS"
            if command["returncode"] == 0
            and gate_report.get("status") == "POST_MANUAL_PIPELINE_FAILED"
            and gate_report.get("post_manual_pipeline_ran") is True
            and preflight_report.get("manual_validation_status") == "PASS"
            and preflight_report.get("launcher_gpu_coverage_ok") is True
            and preflight_report.get("gpu_availability_ok_for_launch") is False
            and preflight_report.get("busy_gpu_indices") == list(range(8))
        ...
        payload = {
            ...
            "gpu_error_seen": gpu_error_seen,
            "command": command,
            "claim_boundary": "GPU guard smoke only; not full robustness evidence",
        }
    finally:
        if not args.keep_temp:
            shutil.rmtree(temp_root, ignore_errors=True)
    (out_dir / "full_robustness_gpu_guard_smoke_report.json").write_text(
        ...
        json.dumps(
            {
                ...
                "gpu_availability_ok_for_launch": payload.get("gpu_availability_ok_for_launch"),
