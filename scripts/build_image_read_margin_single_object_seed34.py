#!/usr/bin/env python3
"""Build single-object images with controlled distance from quadrant boundaries."""

from __future__ import annotations

import csv
from pathlib import Path

from PIL import Image, ImageDraw


OUT_DIR = Path("assets/ume_image_read_margin_single_object_seed34")
SIZE = 512
CENTER = SIZE // 2
SHAPE_SIZE = 64
BG = (148, 148, 148)
OUTLINE = (30, 30, 30)
MARGINS = [8, 24, 48, 88]

TARGETS = [
    ("red circle", "circle", (218, 38, 38)),
    ("blue square", "square", (35, 95, 215)),
    ("yellow triangle", "triangle", (238, 204, 42)),
]

POS_PREFIX = {
    "upper-left": "ul",
    "upper-right": "ur",
    "lower-left": "ll",
    "lower-right": "lr",
}


def draw_shape(draw: ImageDraw.ImageDraw, kind: str, xy: tuple[int, int, int, int], fill: tuple[int, int, int]) -> None:
    if kind == "circle":
        draw.ellipse(xy, fill=fill, outline=OUTLINE, width=3)
    elif kind == "square":
        draw.rectangle(xy, fill=fill, outline=OUTLINE, width=3)
    elif kind == "triangle":
        x0, y0, x1, y1 = xy
        points = [(x0 + (x1 - x0) // 2, y0), (x0, y1), (x1, y1)]
        draw.polygon(points, fill=fill, outline=OUTLINE)
        draw.line(points + [points[0]], fill=OUTLINE, width=3)
    else:
        raise ValueError(f"unknown shape kind: {kind}")


def bbox(position: str, margin: int) -> tuple[int, int, int, int]:
    if "left" in position:
        x1 = CENTER - margin
        x0 = x1 - SHAPE_SIZE
    else:
        x0 = CENTER + margin
        x1 = x0 + SHAPE_SIZE
    if "upper" in position:
        y1 = CENTER - margin
        y0 = y1 - SHAPE_SIZE
    else:
        y0 = CENTER + margin
        y1 = y0 + SHAPE_SIZE
    return x0, y0, x1, y1


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    rows = []
    for margin in MARGINS:
        for object_phrase, kind, color in TARGETS:
            color_name, noun = object_phrase.split()
            for position_phrase, prefix in POS_PREFIX.items():
                key = f"m{margin:03d}_{prefix}_{color_name}_{noun}"
                image = Image.new("RGB", (SIZE, SIZE), BG)
                draw = ImageDraw.Draw(image)
                x0, y0, x1, y1 = bbox(position_phrase, margin)
                draw_shape(draw, kind, (x0, y0, x1, y1), color)
                image_path = OUT_DIR / f"{key}.png"
                image.save(image_path)
                rows.append(
                    {
                        "sample_suffix": key,
                        "object": object_phrase,
                        "target_position": position_phrase,
                        "reference_image": str(image_path),
                        "margin_px": margin,
                        "shape_size": SHAPE_SIZE,
                        "center_x": CENTER,
                        "center_y": CENTER,
                        "distractor_object": "",
                        "distractor_position": "",
                        "ref_x0": x0,
                        "ref_y0": y0,
                        "ref_x1": x1,
                        "ref_y1": y1,
                        "distractor_x0": "",
                        "distractor_y0": "",
                        "distractor_x1": "",
                        "distractor_y1": "",
                    }
                )
    with (OUT_DIR / "reference_layout_metadata.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    print(f"[INFO] wrote {len(rows)} margin single-object images to {OUT_DIR}")


if __name__ == "__main__":
    main()
